/**
 * Google Apps Script for early_bird - Master Sheet Integration
 * Deploy this script specifically to early_bird: 1M3hRWVqCZbF1DhVGuFAzh68pWbLxyF9BZd_ShDrRzKg
 *
 ...
 * 2. Create new project: "early_bird-MasterSheet-Integration"
...
const FORM_CONFIG = {
    ...
    fieldMapping: {
        // EXACT field names from early_bird form
...
function onFormSubmit(e) {
    try {
        console.log('early_bird submission received, processing...');
        ...
        if (result.success) {
            console.log('✅ Successfully wrote early_bird data to master database');
            ...
            console.error('❌ Failed to write early_bird data to master database:', result.error);
...
Form: ${FORM_SOURCE_TAG}
        ...
        //     subject: 'early_bird Validation Failure',
        //     body: emailBody
        // });
...
function mapFormResponse(formResponse, formConfig) {
    ...
    if (mappedData[MASTER_FIELDS.INTEREST_LEVEL]) {
        ...
        const mappings = {
            'ready to sign up and avail the early bird enrollment offer': 'High',
            'like to speak to genwise team to resolve questions on my mind': 'Medium',
            'not interested in the gifted summer program right now': 'Low',
    ...
    // Ensure source tag is set to early_bird
    mappedData[MASTER_FIELDS.SOURCE_TAG] = 'early_bird';
...
function writeToMasterSheet(mappedData) {
    try {
        ...
        console.log(`✅ early_bird data written to master sheet at row ${insertedRow}`);
...
function logSuccess(formData, result) {
    try {
        ...
        console.log('✅ early_bird Success log:', logData);
    } catch (error) {
...
function logError(formData, error) {
    try {
        ...
        console.error('❌ early_bird Error log:', logData);
...
function setupTrigger() {
    try {
        ...
        // Verify this is early_bird
        const expectedFormId = '1IJVMLXOThuQE8WVI4bYiruJrTyxsznTomotkS2rfyYY';
        if (form.getId() !== expectedFormId) {
            throw new Error(`Wrong form! This script is for early_bird (${expectedFormId}), but attached to ${form.getId()}`);
        }
        ...
        console.log('✅ early_bird trigger set up successfully');
        ...
        console.error('❌ Error setting up early_bird trigger:', error.toString());
        return { success: false, error: error.toString() };
    }
}

/**
 * Test function to verify early_bird integration
 */
function testIntegration() {
    try {
        console.log('🧪 Testing early_bird integration... VERSION 2025-10-05-00:20');

        const sampleData = {
            'Child Name': 'Test Early Bird Child',
            'Parent Name': 'Test Early Bird Parent',
            'Parent Email ID': 'test@earlybird.com',
            'Parent Mobile Number': '+1234567890',
            'Interested in the Gifted Summer Program': 'Ready to Sign up and avail the Early Bird Enrollment Offer',
            responseId: 'test-early-response',
        ...
        if (result.success) {
            console.log('✅ early_bird integration test successful!');
            return {
                success: true,
                message: 'early_bird integration working correctly',
                result: result
            };
        } else {
            console.error('❌ early_bird integration test failed:', result.error);
            return {
                success: false,
                message: 'early_bird integration test failed',
                error: result.error
            };
        }

    } catch (error) {
        console.error('❌ early_bird integration test error:', error.toString());
        return {
            success: false,
            message: 'early_bird integration test error',
            error: error.toString()
        };
    }
}