/**
 * FORM-BOUND Google Apps Script for website form
...
function onFormSubmit(e) {
  console.log('📝 Website form submission detected');
  ...
  try {
    ...
    console.log('✅ Website form processing complete');
...
function extractFormData(itemResponses) {
  ...
  itemResponses.forEach(itemResponse => {
    ...
    } else if (question.includes('ready to sign up') || question.includes('interest')) {
      // Map interest level from form response
      if (answer.toLowerCase().includes('ready to sign up') ||
          answer.toLowerCase().includes('very interested')) {
        formData.interest_level = 'High';
      } else if (answer.toLowerCase().includes('learn more') ||
                 answer.toLowerCase().includes('somewhat interested')) {
        formData.interest_level = 'Medium';
      } else {
        formData.interest_level = 'Medium'; // default
      }
    }
  });
...
function sendErrorNotification(error) {
  try {
    const subject = 'Website Form Script Error';
    const body = `
Error in website form processing:
