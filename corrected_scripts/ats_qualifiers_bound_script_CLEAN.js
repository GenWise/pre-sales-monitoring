/**
 * FORM-BOUND Google Apps Script for ats_qualifiers form
...
const EMAIL_CONFIG = {
  ...
  subject: 'New ATS Qualifiers Form Submission'
};
...
function onFormSubmit(e) {
  console.log('📝 ATS qualifiers form submission detected');
  ...
  try {
    ...
    console.log('✅ ATS qualifiers form processing complete');

  } catch (error) {
    console.error('❌ Error processing ATS qualifiers form:', error);
...
function extractFormData(itemResponses) {
  ...
  itemResponses.forEach(itemResponse => {
    ...
    if (question.includes('child') && question.includes('name')) {
    ...
    } else if (question.includes('mobile') || question.includes('phone')) {
      formData.parent_mobile = answer;
    } else if (question.includes('qualify') || question.includes('admission') || question.includes('program')) {
      // Map interest level for ATS qualifiers based on qualification interest
      if (answer.toLowerCase().includes('definitely qualify') ||
          answer.toLowerCase().includes('very interested') ||
          answer.toLowerCase().includes('absolutely')) {
        formData.interest_level = 'High';
      } else if (answer.toLowerCase().includes('probably qualify') ||
        ...
        formData.interest_level = 'Medium';
                 ...
                 answer.toLowerCase().includes('unlikely')) {
        formData.interest_level = 'Low';
      } else {
        formData.interest_level = 'Medium'; // default for qualifiers
      }
    }
  });
...
function sendEmailNotification(formData) {
  try {
    ...
    const body = `
New ATS qualifiers form submission received:
...
function sendErrorNotification(error) {
  try {
    const subject = 'ATS Qualifiers Form Script Error';
    const body = `
Error in ATS qualifiers form processing:
