/**
 * FORM-BOUND Google Apps Script for returning_students form
...
const EMAIL_CONFIG = {
  ...
  subject: 'New Returning Students Form Submission'
};
...
function onFormSubmit(e) {
  console.log('📝 Returning students form submission detected');
  ...
  try {
    ...
    console.log('✅ Returning students form processing complete');

  } catch (error) {
    console.error('❌ Error processing returning students form:', error);
...
function extractFormData(itemResponses) {
  ...
  itemResponses.forEach(itemResponse => {
    ...
    } else if (question.includes('mobile') || question.includes('phone') || question === 'phone number') {
      formData.parent_mobile = answer;
    } else if (question.includes('interest') || question.includes('program') || question.includes('attend') || question.includes('sign up') || question.includes('discount') || question.includes('returning') || question === '') {
      // Map interest level from form response for returning students
      console.log(`DEBUG: Processing interest question: "${question}" = "${answer}"`);
      if (answer.toLowerCase().includes('definitely') ||
          answer.toLowerCase().includes('absolutely') ||
          answer.toLowerCase().includes('yes, we want') ||
          answer.toLowerCase().includes('interested, i want to sign up') ||
          answer.toLowerCase().includes('availing the 30% returning discount')) {
        formData.interest_level = 'High';
        console.log(`DEBUG: Set interest_level to High`);
      } else if (answer.toLowerCase().includes('maybe') ||
                 answer.toLowerCase().includes('considering') ||
                 answer.toLowerCase().includes('interested') ||
                 answer.toLowerCase().includes('have questions')) {
        formData.interest_level = 'Medium';
        console.log(`DEBUG: Set interest_level to Medium`);
      } else if (answer.toLowerCase().includes('not sure') ||
                 answer.toLowerCase().includes('undecided') ||
                 answer.toLowerCase().includes('not interested')) {
        formData.interest_level = 'Low';
        console.log(`DEBUG: Set interest_level to Low`);
      } else {
        formData.interest_level = 'High'; // default for returning students
...
function sendEmailNotification(formData) {
  try {
    ...
    const body = `
New returning students form submission received:
...
function sendErrorNotification(error) {
  try {
    const subject = 'Returning Students Form Script Error';
    const body = `
Error in returning students form processing:
